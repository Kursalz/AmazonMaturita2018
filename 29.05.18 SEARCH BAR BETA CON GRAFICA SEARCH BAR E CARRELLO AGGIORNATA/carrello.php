<?php
session_start();
$_SESSION["paginaChiamante"] = "carrello.php";
if (!$_SESSION["login"]) {
...
<html>
    ...
    <body>
        ...
        <table align=center id="tabellaCarrello" >
            <!--<tr><th>Immagine Prodotto</th><th>Nome Prodotto</th><th>Quantità selezionata</th><th>Costo</th></tr>-->
            <?php
            $nomeStato = "CARRELLO";
            $db = new PDO("sqlite:ECOMMERCE.sqlite");
            if (!$db) {
            ...
            if (!isset($idOrdine)) {
                ...
                $statement = $db->prepare("SELECT * FROM PRODOTTO, PRODOTTIORDINATI WHERE :o=idOrdine AND id=idProdotto AND 0<quantita") or die("Errore nella preparazione del database");
                ...
                while ($row = $statement->fetch()) {
                    ...
                    echo "<td class='cellaCarrello'><a href=\"prodotto.php?prodotto=", $row["id"], "\">";
                    echo "<img height='125' width='125' src='", $row["immagine"], "'>";
                    echo "</a></td>";
                    echo "<td class='cellaCarrello'><a href=\"prodotto.php?prodotto=", $row["id"], "\">";
                    echo "<div name='nomeProdotto'>", $row["nome"], "</div>"; //, ", disponibili: ", $row["quantitaTotale"];
                    echo "</a></td>";
                    echo "<td class='cellaCarrello'>Pezzi disponibili: ", $row["quantita"], "</td>";
                    echo "<td class='cellaCarrello'>Prezzo: &euro; ", intval($row["prezzoUnitario"]) * intval($row["quantita"]), "</td>";
                    echo "</tr>";
                    $_SESSION["subtotal"] += intval($row["prezzoUnitario"]);
                    $_SESSION["total"] += intval($row["prezzoUnitario"]) * intval($row["quantita"]);
                }
                $db = null;
            }
            ?></table>
                            ...
                            <div class="woocommerce">
                                ...
                                <div class="cart-collaterals">
                                    ...
                                    <div class="cart_totals ">
                                        <!--SUBTOTAL= ottenuto sommando il prezzo di ogni singolo prodotto(x1)-->
                                        <!--TOTAL= ottenuto sommando il prezzo per la quantità(xn)-->
                                        ...
                                        <table cellspacing="0">
                                            <tbody>
                                                <tr class="cart-subtotal">
                                                    ...
                                                    <?php
                                                    echo "<td><span class='amount'>€", $_SESSION["subtotal"], "</span></td>";
                                                    ?>
                                                </tr>
                                                <tr class="shipping">
                                                ...
                                                <tr class="order-total">
                                                    <th>Ordine totale</th>
                                                    <?php
                                                    echo "<td><strong><span class='amount'>€", $_SESSION["total"], "</span></strong></td>";
                                                    ?>
                                                </tr>
                                            </tbody>
                                        </table>
